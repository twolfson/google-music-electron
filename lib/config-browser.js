...
window.addEventListener('DOMContentLoaded', function handleDOMLoad () {
  ...
  var configOverrides = JSON.parse(ipc.sendSync('get-config-overrides-sync'));
  ...
  [].slice.call($checkboxContainers).forEach(function bindCheckbox ($checkboxContainer) {
    ...
    if (configOverrides.hasOwnProperty(configItemName)) {
      $checkboxContainer.classList.add('muted');
      $input.disabled = true;
      var $overridenSpan = $checkboxContainer.querySelector('overridden');
      $overridenSpan.classList.remove('hidden');
    }
