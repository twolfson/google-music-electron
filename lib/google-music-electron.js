...
// Define a command to install `mpris`
function installMrpis() {
  ...
  var results = spawnSync(
    'npm', ['install', '--ignore-scripts' /* ignore builds on system's node */].concat(installArgs),
    {stdio: 'inherit'});
  ...
  results = spawnSync(electronRebuildCmd, {stdio: 'inherit'});
...
program
  ...
  .command('install-mpris', 'Install integration with MPRIS (Linux only)').action(installMrpis)
