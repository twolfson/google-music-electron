...
// If we were requested to minimize to tray, then replace `toggleVisibility` with a show/hide equivalent
if (program.closeToTray) {
  gme.toggleVisibility = function () {
    if (gme.browserWindow) {
...
app.on('ready', function handleReady () {
  ...
  // When a minimization is being requested, trigger a visibility toggle instead
  // DEV: This is to allow us to force show/hide behavior
  gme.browserWindow.on('minimize', function handleBrowserMinimize (evt) {
    // Stop the default action and replace it with a visibility toggle
    evt.preventDefault();
    gme.toggleVisibility();
  });
