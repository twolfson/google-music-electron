...
var gme = {
  ...
  toggleFocusMinimize: function () {
    if (gme.browserWindow) {
      var isFocused = gme.browserWindow.isFocused();
      var isMinimized = gme.browserWindow.isMinimized();
      logger.debug('Toggling browser window minimization/focus', {
        isFocused: isFocused,
        isMinimized: isMinimized
      });
      if (!isMinimized && isFocused) { // Visible and focused
        gme.browserWindow.minimize();
      } else if (!isMinimized && !isFocused) { // Visible and not focused
        gme.browserWindow.focus();
  ...
  toggleFocusVisibility: function () {
    if (gme.browserWindow) {
      var isFocused = gme.browserWindow.isFocused();
      var isVisible = gme.browserWindow.isVisible();
      logger.debug('Toggling browser window visibility/focus', {
        isFocused: isFocused,
        isVisible: isVisible
      });
...
gme.onTrayClick = (config.get('skip-taskbar') || config.get('hide-via-tray') || config.get('minimize-to-tray')) ?
  gme.toggleFocusVisibility : gme.toggleFocusMinimize;
gme.onRaise = (config.get('skip-taskbar') || config.get('hide-via-tray') || config.get('minimize-to-tray')) ?
...
function launchGme() {
  ...
  if (config.get('minimize-to-tray')) {
    gme.browserWindow.on('minimize', gme.toggleFocusVisibility);
  }
