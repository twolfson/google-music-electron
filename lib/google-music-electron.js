...
var googleMusicElectron = {
  ...
  controlPlayPause: function () {
    var browserWindow = googleMusicElectron.browserWindow;
...
app.on('ready', function handleReady () {
  ...
  googleMusicElectron.browserWindow = new BrowserWindow(windowOpts);
  googleMusicElectron.browserWindow.loadUrl('https://play.google.com/music/listen');

  // When our window is closed, clean up the reference to our window
  googleMusicElectron.browserWindow.on('closed', function handleWindowClose () {
    logger.debug('Browser window closed, garbage collecting `browserWindow`');
    googleMusicElectron.browserWindow = null;
  });

  // Save browser window context to replify
  replify('google-music-electron', null, {googleMusicElectron: googleMusicElectron});
