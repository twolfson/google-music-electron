...
exports.init = function (gme) {
  ...
  function bindMenuItems(menuItems) {
    menuItems.forEach(function bindMenuItemFn (menuItem) {
      ...
      // here in the last condition, we also verify if type isn't 'separator'
      // because ijn this `separator` cases we will have no command but still
      // want the menu item (or just a separator in this case), to be functional
